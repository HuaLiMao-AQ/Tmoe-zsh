...
check_linux_distro() {
	if grep -Eq 'debian|ubuntu' "/etc/os-release"; then
		...
		if [ ! -e "router-zsh.bash" ]; then
			wget --no-check-certificate -qO "router-zsh.bash" https://gitee.com/mo2/zsh/raw/master/zsh.sh
		fi
	...
	if [ ! -z "${DEPENDENCIES}" ]; then
		...
		if [ "$(id -u)" != "0" ]; then
			if [ $(command -v curl) ]; then
				sudo -E bash -c "$(curl -LfsS https://gitee.com/mo2/zsh/raw/master/zsh.sh)" ||
					su -c "$(curl -LfsS https://gitee.com/mo2/zsh/raw/master/zsh.sh)"
			else
				sudo -E bash -c "$(wget -qO- https://gitee.com/mo2/zsh/raw/master/zsh.sh)" ||
					su -c "$(wget -qO- https://gitee.com/mo2/zsh/raw/master/zsh.sh)"
			fi
			exit 0
		fi
...
termux_git_clone_zsh() {
	...
	if [ ! -d "${HOME}/.termux-zsh" ]; then
		git clone https://gitee.com/mo2/zsh.git "${HOME}/.termux-zsh" --depth 1
...
tmoe_zsh_main_menu() {
	...
	TMOE_OPTTION=$(whiptail --title "TMOE-ZSH manager running on Linux.(20200813)" --backtitle "Please select onekey configuration for initial installation." --menu "输zsh-i启动本工具,type zsh-i to start this tool.\nPlease use the enter and arrow keys to operate.\n请使用方向键和回车键进行操作,初次安装请选择一键配置" 0 50 0 \
...
tmoe_zsh_faq() {
	...
	#20 50 7
	TMOE_OPTTION=$(whiptail --title "FAQ(よくある質問)" \
...
ItemizedConfiguration() {
	...
	TMOE_OPTTION=$(whiptail --title "Itemized Configuration" --menu "您想要配置哪个项目?zsh主题全平台通用,Termux配色和字体仅适用Android Termux,xfce4终端配色仅适用于linux。You can use zsh theme on all platforms,but termux colors and fonts are only available for Android." 17 50 5 \
...
REMOVEZSH() {
	...
	TMOE_OPTTION=$(whiptail --title "REMOVE ZSH" --menu "您想要移除哪个项目？\nWhich item do you want to remove?" 0 50 0 \
		"0" "🌚 Back to the main menu 返回主菜单" \
		"1" "oh-my-zsh" \
		"2" "fonts 字体" \
		"3" "tmoe-zsh 工具" \
...
##########
download_tmoe_zsh() {
	ZSH_I_URL='https://gitee.com/mo2/zsh/raw/master/zsh.sh'
...
git_clone_oh_my_zsh() {
	...
	if [ ! -d "${HOME}/.oh-my-zsh" ]; then
		git clone https://gitee.com/mirrors/oh-my-zsh.git "${HOME}/.oh-my-zsh" --depth 1 || git clone --depth=1 git://github.com/ohmyzsh/ohmyzsh "${HOME}/.oh-my-zsh"
	...
	if [ ! -f "${HOME}/.zshrc" ]; then
		cp "${HOME}/.oh-my-zsh/templates/zshrc.zsh-template" "${HOME}/.zshrc" || wget -O "${HOME}/.zshrc" 'https://gitee.com/mirrors/oh-my-zsh/raw/master/templates/zshrc.zsh-template'
